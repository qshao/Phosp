...
class MDEngine(ABC):
    ...
    @abstractmethod
    def run_phase(self, phase: str, mdp: Path, topology: Path, structure: Path, output_dir: Path, restraint_gro: Path | None = None, gpu_id: int | None = None) -> SimulationResult: ...

    @abstractmethod
    def generate_hpc_script(self, scheduler: str, resources: dict, phases: list[str], output_dir: Path, work_dir: Path | None = None) -> Path: ...
